...
export async function createSubject(req: Request, res: Response) {
  ...
  try {
    const created = await service.createSubjectRecord({
      schoolId,
      name,
      code,
    ...
    if (err?.code === "P2002")
      return res
        .status(400)
        .json({ error: "Disciplina com esse código já existe na escola" });
...
export async function updateSubject(req: Request, res: Response) {
  ...
  const updated = await service.updateSubjectById(id, {
    name: name ?? undefined,
