...
const PERIOD_STATUS_COLORS: Record<string, "green" | "gray"> = {
  OPEN: "green",
  CLOSED: "gray",
};
const YEAR_STATUS_OPTIONS = Object.entries(YEAR_STATUS_LABELS).map(
  ([v, l]) => ({ value: v, label: l }),
);
...
export default function AcademicYears() {
  ...
  const [modal, setModal] = useState<
    "createYear" | "editYear" | "status" | "periods" | "createPeriod" | null
  >(null);
  ...
  const load = useCallback(async (p = 1) => {
    ...
    try {
      const res = await api.fetchJson(
        `/academic-years?page=${p}&limit=${LIMIT}`,
      );
      setYears(res?.data ?? res ?? []);
      setTotal(res?.meta?.total ?? 0);
      setPage(p);
    } catch {
      toast("Erro ao carregar anos letivos", "error");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function loadPeriods(yearId: string) {
    try {
      ...
      toast("Erro ao carregar períodos", "error");
    }
  }
  ...
  async function handleCreateYear(e: React.FormEvent) {
    ...
    if (yearNum <= 0 || isNaN(yearNum)) {
      setFormError("Ano deve ser maior que 0.");
      return;
    }
  ...
  async function handleActivate(year: AcYear) {
    try {
      await api.fetchJson(`/academic-years/${year.id}/activate`, {
        method: "PATCH",
      });
      toast("Ano letivo ativado!");
      load(page);
    } catch (e: any) {
      toast(e?.message || "Erro ao ativar.", "error");
    }
  }
  ...
  async function handleCreatePeriod(e: React.FormEvent) {
    ...
    try {
      ...
      toast("Período criado!");
      setModal("periods");
      loadPeriods(selected.id);
    } catch (e: any) {
      setFormError(e?.message || "Erro ao criar período.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function togglePeriodStatus(period: Period, yearId: string) {
    try {
      ...
      await api.fetchJson(
        `/academic-years/${yearId}/periods/${period.id}/status`,
        {
          method: "PATCH",
  ...
  const rows = years.map((y) => [
    ...
    <StatusBadge
      label={YEAR_STATUS_LABELS[y.status] || y.status}
      color={YEAR_STATUS_COLORS[y.status] || "gray"}
    />,
    y.active ? (
      <StatusBadge label="Ativo" color="green" />
    ) : (
      <StatusBadge label="Inativo" color="gray" />
    ),
  ...
  const periodRows = periods.map((p) => [
    ...
    <StatusBadge
      label={PERIOD_STATUS_LABELS[p.status] || p.status}
      color={PERIOD_STATUS_COLORS[p.status] || "gray"}
    />,
    <IconButton
      title={p.status === "OPEN" ? "Fechar" : "Reabrir"}
  ...
  return (
    ...
    >
        ...
        <form onSubmit={handleCreateYear}>
          ...
          <FormField label="Data de início" required>
            <input
              ...
              onChange={(e) =>
                setYearForm((f) => ({ ...f, startDate: e.target.value }))
              }
              style={inputStyle}
            />
          </FormField>
      ...
      {/* Create period */}
      <Modal
        open={modal === "createPeriod"}
        onClose={() => setModal("periods")}
        title="Novo período"
      >
        <form onSubmit={handleCreatePeriod}>
          ...
          <FormField label="Sequência">
            <input
              ...
              onChange={(e) =>
                setPeriodForm((f) => ({ ...f, sequence: e.target.value }))
              }
              style={inputStyle}
            />
          </FormField>
          ...
          <ModalFooter>
            <PrimaryButton variant="ghost" onClick={() => setModal("periods")}>
              Voltar
            </PrimaryButton>
            <PrimaryButton type="submit" loading={saving}>
              Criar período
